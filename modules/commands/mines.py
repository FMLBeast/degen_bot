"""
Mines calculator commands with topic support
"""
import math
...
async def mines_multi_command(update, context):
    """Calculate mines multipliers and probabilities"""
    ...
    if len(args) == 2:
        try:
            m = int(args[0])  # mines
            d = int(args[1])  # diamonds
        except ValueError:
            await reply_to_message(update, context, "Please provide valid numbers for mines and diamonds.")
            return

        if m + d > 25 or m <= 0 or d <= 0:
            await reply_to_message(update, context, "Mines and diamonds total must be less than or equal to 25, and both must be greater than 0.")
            return
        ...
        try:
            ...
            await reply_to_message(update, context, "Please provide a valid multiplier.")
            return
        ...
        await reply_to_message(update, context, "Usage: /mines_multi <multiplier> or /mines_multi <mines> <diamonds>")